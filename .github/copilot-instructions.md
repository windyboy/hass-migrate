...
- **Database Clients** (`hass_migrate/database/`): `MySQLClient` and `PGClient` for connections, using aiomysql and asyncpg.
...
- **Testing**: `uv run pytest` for all tests; `uv run pytest tests/test_file.py::TestClass::test_method` for specific; `uv run pytest --lf` for last failed. E2E tests in `test_e2e.py`.
...
## Integration Points
- **External Deps**: asyncpg (PG), aiomysql (MySQL), typer (CLI), rich (console), pydantic (config).
