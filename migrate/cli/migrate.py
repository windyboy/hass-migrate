...
from migrate.cli.constants import PROGRESS_FILE, TABLES, console, logger
...
def register_migrate_commands(migrate_app: typer.Typer) -> None:
    ...
    def migrate_table(
        ...
        batch_size: int = typer.Option(
            ...
            help="Number of rows per batch (default: 20000)",
        ),
            ...
            try:
                ...
                if not force:
                    if not typer.confirm(
                        f"Truncate target table '{table_name}' before migration?"
                    ):
                ...
                config = MigrationConfig(
                    ...
                    progress_update_interval=cfg.progress_update_interval,
                ...
                if result.success:
                    ...
                    console.print(
                        f"[bold red]Done with errors: {table_name}[/bold red]"
                    )
                    raise typer.Exit(1)
            finally:
    ...
    async def _perform_migration(
        ...
        schema_name = get_schema_name(cfg, schema)
        pg_client = PGClient(cfg, schema=schema_name)
        backup_service = BackupService(logger) if backup else None
        ...
        try:
            ...
            # Create backup if requested and not resuming
            ...
            if resume:
                if not os.path.exists(PROGRESS_FILE):
                    console.print(
                        "[yellow]Progress file not found, starting fresh migration.[/yellow]"
                    )
                else:
            ...
            # Truncate tables if not resuming
            if not resume:
    ...
    def migrate_all(
        ...
        max_concurrent: int = typer.Option(
            4,
            "--max-concurrent",
            callback=validate_max_concurrent,
            help="Maximum number of tables to migrate concurrently (default: 4)",
        ),
        backup: bool = typer.Option(
            False,
            "--backup",
            help="Create backup before migration (ignored if --resume is used)",
        ),
        schema: str = get_schema_option(),
    ):
    ...
    def migrate_resume(
        ...
        if not os.path.exists(PROGRESS_FILE):
            console.print(
                "[red]Error: No progress file found. Cannot resume migration.[/red]"
            )
            raise typer.Exit(1)
